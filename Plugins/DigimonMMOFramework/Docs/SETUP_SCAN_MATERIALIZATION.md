...
## Goal
The native framework now supports the complete account-persistent loop: **defeat eligible Wild Digimon -> earn species Scan Data -> reach threshold -> materialize a unique permanent Digimon -> manage/summon it from Collection**. All mutations are server-authoritative.
...
- Collection portrait (existing v0.6.0 behavior)
...
7. Review progress, cap, reward, owned count and Collection occupancy.
8. Press `MATERIALIZE DIGIMON`.
9. Server validates the request, creates a unique owned Digimon, consumes the configured requirement, saves immediately and switches the menu back to Collection.
10. Use `SET ACTIVE / SUMMON` exactly like any starter/owned Digimon.

## 6. Persistence and multiplayer
Scan Data is copied to/from `FDMFAccountRecord::ScanData`, replicated `COND_OwnerOnly`, and saved with the same account persistence path as Collection/Money. One player cannot see or materialize another player's scan progress. The server performs every reward and materialization mutation.
...
## 8. Acceptance test
Use one Wild Gabumon with reward 20 / cap 100 / requirement 100. Defeat five separate eligible Gabumon and verify the owner sees 20, 40, 60, 80, 100. Materialize once and verify Scan returns to 0, the Gabumon remains listed in the Scan Database, one new unique Gabumon appears in Collection, and it can be summoned normally. Reconnect and verify both the 0% scan record and materialized Digimon persist. Then repeat with host + remote client and confirm only the player whose partner earned the victory receives Scan Data.
