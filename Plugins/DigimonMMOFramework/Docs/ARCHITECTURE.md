...
## v0.19.1 persistent Social authority and nearby-player discovery

Social deliberately follows the framework's authenticated-persistence model rather than making relationship data public PlayerState replication. `FDMFAccountRecord` schema v8 owns friendships, reciprocal inbound/outbound pending friend requests, Ignore entries, tracked-friend preferences, current GuildId and inbound guild invitations. The same `UDMFAccountDatabaseSaveGame` owns a global `TMap<FGuid,FDMFGuildRecord>` registry for guild identity, owner, membership and pending applications.

`ADMFMMOGameMode` is the mutation authority. `DMFMMOPlayerController` exposes strongly named Blueprint functions, but they collapse to one validated reliable social-mutation RPC so the network surface remains small. The GameMode resolves the actor from `AuthenticatedUsername`, reloads every target/guild from persistence, applies role/capacity/ignore/name rules, then calls `SaveSocialTransaction`. That helper commits all touched accounts/guilds in one SaveGame write and restores both in-memory maps if disk persistence fails.

The client receives an owner-only `FDMFSocialSnapshot`; private friend/ignore/request/guild-management data is never copied into a public replicated PlayerState property. Login/logout presence refreshes push updated online/offline flags only to relevant online friends/guild members.

The presentation side remains native-UI-first and replaceable. `UDMFDigimonInventoryWidget` contains the top-level SOCIAL tab and extensible nested Friends/Guild pages. The Friends page includes an owner-local Nearby Players discovery panel backed by `ADMFMMOPlayerController::GetNearbySocialPlayers()`: replicated player avatars are filtered by the Project Settings radius, deduplicated by public username and sorted nearest-first. This derived list has no persistent or network authority; ADD/ACCEPT/CANCEL still route through the same server Social mutation endpoint. `UDMFPlayerSocialContextWidget` remains an optional owner-local nameplate shortcut. `UDMFFriendTrackerWidget` is created as a non-replicated screen-space Widget Component only for tracked online friends. Both discovery and tracker distance read already replicated actor transforms, avoiding distance/discovery RPCs or replicated waypoint ticks.

Ignore is implemented at the WORLD-chat delivery boundary. Player actors continue to replicate normally. Only `EDMFWorldChatMessageType::Player` messages authored by an ignored sender are omitted for that recipient, including server history hydration; server-authored presence events remain separate message types.
