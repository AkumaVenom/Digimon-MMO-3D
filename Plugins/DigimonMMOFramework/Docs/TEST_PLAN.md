...
### Native Social shell, Nearby Players and optional nameplate context
1. Start a listen host and at least two separate authenticated client accounts. Open the native **DIGIMON MENU → SOCIAL** for the first time and confirm **FRIENDS & IGNORE** is selected by default. Confirm the page now presents **NEARBY PLAYERS**, **FRIENDS**, **FRIEND REQUESTS** and **IGNORE LIST** as four clean columns. Switch repeatedly between **FRIENDS & IGNORE** and **GUILD** and confirm Guild behavior/layout is unchanged.
2. Set **Nearby Player Friend Discovery Radius** to a small obvious value such as `10 m`. Stand B at 3 m and C at 8 m from A; A's Nearby Players list must show B first then C with rounded integer metre values. Move B farther than C without leaving radius and verify the list reorders nearest-first within the configured refresh interval. Move each player beyond 10 m and verify that row automatically disappears; move back inside and verify it returns. The local player's own account must never appear.
3. From A's Nearby Players list select **ADD FRIEND** on B. B must receive an INCOMING Friend Request without any modal popup; A's nearby row changes to REQUEST SENT / CANCEL after the authoritative snapshot arrives. Have B select **ACCEPT** either from B's Nearby Players row or Friend Requests queue and confirm both become persistent friends. Confirm the row becomes a non-action FRIEND state while the separate **IGNORE** action remains available. Select **IGNORE** directly from that nearby row and confirm the existing friendship/request state is cleaned authoritatively, B remains physically visible in the world and nearby list, the row becomes **IGNORED** with **UNIGNORE**, and B's authored WORLD chat is filtered for A. Select **UNIGNORE** from the same nearby row and confirm normal future B chat resumes.
4. Change **Nearby Player List Refresh Interval** and verify the visual cadence changes without any increase in framework RPC count or server-side polling. `GetNearbySocialPlayers()` must use normal replicated avatars/transforms only. Disable Player Nameplate Social Context entirely and confirm Nearby Players friend discovery still works end-to-end.
5. Re-enable nameplate context and click another player's replicated native nameplate. Confirm the optional owner-local **PLAYER ACTIONS** surface still behaves as v0.19.0 when hit testing is available. Clicking your own nameplate must not open it. A client must never be able to submit another player's acting username to authority.
6. Verify the Social master switch disables Social presentation/action entry points without affecting player replication, WORLD chat, combat, Party/Bank or nameplates themselves.

### Friends, requests and persistent tracking
7. Account A chooses **ADD FRIEND** on B. B must receive no popup: the request appears as **INCOMING** in B's **SOCIAL → FRIENDS & IGNORE → FRIEND REQUESTS**, while A sees the same transaction as **OUTGOING** and B's nameplate context becomes **CANCEL REQUEST**. Cancel once and verify both sides clear atomically. Send again, disconnect B without answering, reconnect to the still-running host and confirm both persisted request views are still coherent.
8. Decline one request and confirm it disappears from both accounts persistently. Send another request and accept it. A and B must both gain the other in their Friends list through one server-authoritative persistent transaction. Reconnect both accounts and confirm the symmetric friendship remains.
9. Remove the friendship from either account. Confirm both accounts are updated, any stored tracking preference for that relationship is removed, and reconnect cannot resurrect the stale friend.
10. Recreate the friendship and toggle **TRACK** for B only on A. Confirm A receives one extra owner-local friend marker over B showing B's name and an integer distance in metres. Move both players and confirm the value updates from already replicated transforms without any distance RPC. B must not automatically see a tracker for A.
11. Toggle **HIDE TRACKER**, then enable it again and reconnect A. Confirm the persistent per-friend tracking preference restores when B is online, disappears cleanly while B is offline/missing, and reconstructs when B returns. Confirm there is never more than one tracker component for the same friend.

### Ignore and WORLD-chat filtering
12. A chooses **IGNORE PLAYER** on B. Confirm B remains completely visible/replicated in the world and all normal gameplay presence remains intact, but new authored `Player` WORLD-chat messages from B are not delivered to A. Other players must still receive B's chat normally.
13. Generate B chat while A is disconnected/ignored, then reconnect A and request normal WORLD-chat history. B's authored player messages must also be filtered from history hydration. Server-authored join/leave presence and other System messages must remain visible.
14. Ignore must also break any existing A↔B friendship, associated tracker/pending friend requests, direct guild invitations and owner-facing guild applications between those accounts atomically. While either side ignores the other, new guild invites/applications must be rejected. Remove B from A's Ignore list and confirm future B chat is delivered again; previously filtered history does not need to be retroactively injected. Reconnect and verify the unignore persists.

### Guild lifecycle, invitations, directory and offline applications
15. From an account with no guild, enter a valid name in **SOCIAL → GUILD** and create a guild. Confirm the creator is the persistent owner and member, the guild appears in the searchable directory, and duplicate names are rejected case-insensitively. Exercise configured minimum/maximum name lengths and invalid characters.
16. As owner, rename the guild. Confirm the new name persists after host/account relog and updates the directory/member snapshots on other already-connected clients without requiring them to close the Social menu. A non-owner must not be able to rename, disband, accept applications or remove members even if it invokes the Blueprint/C++ wrapper directly.
17. As guild owner, click an unaffiliated player's nameplate and choose **INVITE TO GUILD**. The target receives no popup; the invite appears in **GUILD INVITES** and survives disconnect/relog. Test Decline, then invite again and Accept. Authority must revalidate guild existence, capacity and target membership at acceptance time.
18. After acceptance, both owner and member lists update and persist. The owner removes the member and the removed account's GuildId clears persistently. Rejoin, invite/accept again, then have the normal member choose **LEAVE GUILD**; owner/member snapshots and persistence must again converge.
19. Create a second guild/account. Use the **GUILD DIRECTORY** search box to filter by guild name and owner name. An unaffiliated player applies to a guild while its owner is offline. Restart/reconnect the owner later and confirm the application is waiting in **JOIN APPLICATIONS**, with no popup spam.
20. Decline one application and verify persistent removal. Submit another and Accept. Confirm the applicant joins atomically, disappears from other pending guild applications/invites as appropriate, and member capacity cannot be exceeded by racing an invitation acceptance against an application acceptance.
21. As guild owner, **DISBAND GUILD**. Confirm the guild disappears from the directory, every member account loses that GuildId, pending invites referring to the deleted guild are cleared, online affected users receive refreshed Social snapshots, and reconnecting offline affected accounts does not restore the deleted guild.

### Authority, persistence, load and regression
22. Spam Social mutation actions faster than the configured **Minimum Social Action Interval** and verify server throttling rejects excess changes without partially mutating accounts/guilds. Also spam explicit `RequestSocialSnapshot` calls faster than **Minimum Social Snapshot Request Interval** and confirm authority silently bounds read work while normal server-pushed updates still arrive. Test configured friend, ignore, guild-member, pending-guild-invite and pending-guild-application caps.
23. Inspect/attempt malformed calls: self-friend/self-ignore, unknown account, stale guild GUID, accepting a stale request, joining two guilds, non-owner membership mutation, oversized text payload and client-supplied casing variants. Every invalid path must fail on server authority and leave persistent records coherent.
24. Exercise simultaneous cross-account changes from two clients where practical. A persistence failure or rejected action must not leave a one-sided friendship, partial guild membership or half-cleared GuildId. `SaveSocialTransaction` is expected to commit all changed account/guild records through one SaveGame flush or restore its in-memory maps on failure.
25. Re-run the accepted v0.18.5 join/relog/leave WORLD-chat announcements + sounds, v0.18.4 global capacity, v0.18.3 vendor BUY/SELL, v0.18.2 same-host reconnect/disconnect save-before-teardown, Party/Bank privacy, player-world-location restore, Return Home, swimming/underwater, Day/Night, combat, DigiDex, Digivolution and Care. Existing gameplay saves must load through the v7→v8 migration with all prior data intact. Candidate RPC count is **53**.
---
